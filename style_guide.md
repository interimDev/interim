...
* Follow the title with " - " bullet points for any further information we'll need about the commit.
...
* When updating code forked another project (in this case, Firechat), preface that update comment. Tag the team name with brackets [] to clearly denote original code from team updates.

  ```javascript
    // good:
    // [Interim] Added tag option to message content
    Firechat.prototype.sendMessage = function(roomId, messageContent, tag, messageType, cb) {};


    // bad:
    // Added tag option to message content
    Firechat.prototype.sendMessage = function(roomId, messageContent, tag, messageType, cb) {};
    ```
